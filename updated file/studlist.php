    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save'])) {
        ...
        for ($i = 0; $i < count($_POST['id']); $i++) {
         ...
         // Determine interventions based on major and minor offenses
        $majorCount = intval($majorValue);
        $minorCount = intval($minorValue);

        if ($majorCount == 1) {
            $interventionsValue = "1st Warning";
        } elseif ($majorCount == 1 && $minorCount == 2) {
            $interventionsValue = "2nd Warning";
        } elseif ($majorCount == 2 && $minorCount == 1) {
            $interventionsValue = "3rd Warning";
        } elseif ($majorCount >= 3 || ($majorCount > 0 && $minorCount > 0)) {
            $interventionsValue = "Drop Out";
        } else {
            $interventionsValue = "";  // Set a default value if no condition matches
        }

        $sql = "UPDATE record SET lrn = '$lrnValue', level = '$levelValue', majoroffense = '$majorValue', minoroffense = '$minorValue', status = '$statusValue', status = '$interventionsValue' WHERE id = '$idValue'";
